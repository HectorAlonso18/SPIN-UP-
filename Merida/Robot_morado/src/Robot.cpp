...
void Robot:: Flywheel_PID (void*ptr){
    ...
    while(1){
        ...
        //En caso que el anterior target y el actual sean el mismo
        //seguir la misma secuencia y poder manipular el valor máximo para evitar el sobretiro
        ...
        if(!(abs(FLYWHEEL.error)   <50)){
            ...
            //Cuanos se estabiliza debemos de limitar el maximo al valor del final power para no contar con sobretiro 
        else{
            FLYWHEEL.maximo = FLYWHEEL.finalpower;
            Ready_to_shoot=true; 
        }*/
...
void Robot::drive(void*ptr){
    ...
    while(true){
        ...
        shoot_configuration = master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R1)==1 ? shoot_configuration+=1: shoot_configuration>4 ? 1 : shoot_configuration;
        
        light_configuration=shoot_configuration; 
        Turn_lights(light_configuration);

        RPM = shoot_configuration==1 ? LONG_FLYWHEEL_VELOCITY : shoot_configuration==2 ? MEDIUM_FLYWHEEL_VELOCITY : shoot_configuration == 3 ? SHORT_FLYWHEEL_VELOCITY: shoot_configuration==4 ||expansiooon==THROW_EXPANSION ? STOP_FLYWHEEL_VELOCITY : RPM;  
